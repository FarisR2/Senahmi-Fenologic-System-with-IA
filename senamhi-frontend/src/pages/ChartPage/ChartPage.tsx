...
const ChartPage = () => {
  ...
  if (filteredAnalytics.length > 0) {
    ...
    const endDate = new Date(Date.UTC(endYear, endMonth - 1, 31));
    let currentDate = new Date(startDate);

    while (currentDate <= endDate) {
      const formattedDate = `${currentDate.getUTCDate().toString().padStart(2, '0')}/${(currentDate.getUTCMonth() + 1).toString().padStart(2, '0')}`;
    ...
    filteredAnalytics.forEach(analytic => {
      ...
      datesArray.forEach((dateStr, index) => {
        ...
        const formattedDate = `${date.getUTCDate().toString().padStart(2, '0')}/${(date.getUTCMonth() + 1).toString().padStart(2, '0')}`;
