...
export const AnalyticForm = ({ onPost, showSuccess, stations, cultives, fenologics }: Props) => {
  ...
  const [selectedStationId, setSelectedStationId] = useState<string | number>('');
  const [selectedCultiveId, setSelectedCultiveId] = useState<string | number>('');
  ...
  const filteredFenologics = fenologics.filter(f => f.cultiveId === Number(selectedCultiveId));

  // Obtener cultivo seleccionado para usar su intervalo de días
  const selectedCultive = cultives.find(c => c.id === Number(selectedCultiveId));
