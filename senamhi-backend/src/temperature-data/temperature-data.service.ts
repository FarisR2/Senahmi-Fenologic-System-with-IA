...
export class TemperatureDataService {
  ...
  async createTemperatureData(
    dto: CreateTemperatureDataDto,
  ): Promise<TemperatureDataEntity> {
    ...
    const savedTempData =
      await this.temperatureDataRepository.save(newTempData);

    // Emitir evento
    this.eventEmitter.emit('temperature.uploaded', savedTempData);

    return savedTempData;
  }
  ...
  async findByStationAndMonth(
    stationId: number,
    month: number,
    year: number,
  ): Promise<TemperatureDataEntity | null> {
  ...
  async findOne(id: number): Promise<TemperatureDataEntity> {
    ...
    if (!item)
      throw new NotFoundException(`Temperature data with ID ${id} not found`);
    return item;
  }
  ...
  async getUploadedMonths(
    stationId: number,
    year: number,
  ): Promise<{ uploadedMonths: number[]; files: Record<number, string> }> {
